...
dice_combos = {0: (0, 2, 0, 2, "Unskilled PC vs Average Difficulty"),
               1: (0, 3, 0, 2, "Unskilled PC vs Hard Difficulty"),
               2: (2, 1, 0, 2, "Unskilled PC vs Advesary NPC"),
               3: (0, 2, 0, 4, "Talented PC vs Average Difficulty"),
               4: (0, 3, 0, 4, "Talented PC vs Hard Difficulty"),
               5: (2, 1, 0, 4, "Talented PC vs Advesary"),
               6: (0, 2, 3, 1, "Pro PC vs Average Difficulty"),
               7: (0, 3, 3, 1, "Pro PC vs Hard Difficulty"),
               8: (2, 1, 3, 1, "Pro PC vs Advesary NPC"),
               }
...
for variable in dice_combos:
    ...
    if dice_chal:
        ...
        graph["symbols"] = "%s%s" % (graph["symbols"], "c" * dice_chal)
        graph["color_list"].append("#ff0000")
    if dice_diff:
        dice = "%s, %d Difficulty" % (dice, dice_diff)
        graph["symbols"] = "%s%s" % (graph["symbols"], "d" * dice_diff)
    if dice_prof:
        dice = "%s, %d Proficiency" % (dice, dice_prof)
        graph["symbols"] = "%s%s" % (graph["symbols"], "C" * dice_prof)
        graph["color_list"].append("#ffff00")
    if dice_abil:
        dice = "%s, %d Ability" % (dice, dice_abil)
        graph["symbols"] = "%s%s" % (graph["symbols"], "D" * dice_abil)
    dice = dice.strip(", ")
    ...
    graph["x_breaks"] = ([range_bottom - 0.5, ] +
                         range(range_bottom, range_top) + [range_top - 0.5, ])
    graph["x_labels"] = ([" ", ] +
...
def update_plot(gkey, graph_conf, plot):
    ...
    breaks = len(graph_conf["x_breaks"])
